...
#%%
## load configs
parser = argparse.ArgumentParser(description="PCBM")
parser.add_argument('--split', type=int, default=6)
parser.add_argument('--conceiver_path', type=str, default=f'logs/conceiver6/model.t7')
parser.add_argument('--observer_path', type=str, default=f'logs/nnn6/model.t7')
parser.add_argument('--predictor_path', type=str, default=f'logs/plain_predictor6/model.t7')
...
## build the model
observer = models.DTUNet(in_channels, seg_channel)

conceiver = models.FetalConceiver(
...
def validate_one_epoch(loader):
    ...
    for x in tqdm(loader):
        ...
        with torch.no_grad():
            assign_mtx = observer({'image':image})['logit']
        ...
        with torch.no_grad():
            ...
            concept_logit[:, [2, 4, 8, 13, 17, 24]] = torch.sigmoid(concept_logit[:, [2, 4, 8, 13, 17, 24]])
