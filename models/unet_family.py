...
class RegUNet(nn.Module):
    def __init__(self, in_channels, num_classes, model_size = '016', model_type= 'y', interpolation=True, extra_fc = False, **kwargs):
        super().__init__()
        ...
        if extra_fc:
            self.up_conv0 = DeConvBlock(0, channels[-5], out_channels=channels[-5], interpolation=interpolation)
            self.fc = nn.Conv2d(channels[-5], num_classes, kernel_size=1)
        else:
            self.up_conv0 = DeConvBlock(0, channels[-5], out_channels=num_classes, interpolation=interpolation, **kwargs)
...
class ResUNet(nn.Module):
    def __init__(self, in_channels, num_classes, model_size = 'resnet18', init_channels = 64, interpolation=True, **kwargs):
        ...
        #                 nn.ConvTranspose2d(channels[-5], channels[-5], kernel_size=2, stride=2),
        #                 nn.LeakyReLU()
        #                 )
    ...
    def forward(self, x):
        ...
        up0 = nn.functional.interpolate(up1, (up1.size(2)*2, up1.size(3)*2))
...
class FlexUNet(nn.Module):
    def __init__(self, in_channels, out_channels, pair_num=4, interpolation=True):
        ...
        init_channels = 64
        self.first_conv = ConvBlock(in_channels, init_channels, activation=None)
        ...
        for i in range(pair_num):
            left_conv, right_conv = self._mask_pair_conv(init_channels, interpolation)
            init_channels *= 2
    ...
    def _mask_pair_conv(self, init_channels, interpolation):
        left_conv = nn.Sequential(
            nn.MaxPool2d(kernel_size=2, stride=2),
            ConvBlock(init_channels, init_channels*2, activation=None)
            )
        right_conv =  DeConvBlock(init_channels, init_channels*2, init_channels, interpolation=interpolation)
...
class DTUNet(nn.Module):
    def __init__(self, in_channels, out_channels, interpolation=True): # , quality_head=True
        ...
        self.unet = RegUNet(in_channels, num_classes=out_channels, interpolation=interpolation, model_size='016', extra_fc=True)
        # self.unet = RegUNet(in_channels, num_classes=out_channels, interpolation=interpolation, model_size='004', extra_fc=True)
        # self.unet = ResUNet(in_channels, num_classes=out_channels, interpolation=interpolation, model_size='resnet18', dropout=0.2)
        # self.unet = RegUNet(in_channels, num_classes=out_channels, interpolation=interpolation, model_size='016', extra_fc=False)
    ...
    def forward(self, x):
        ...
        if self.training and x['epoch'] >= start_epoch: # during training, activate triplet loss
            ...
            lambda_ = 0.8-(x['epoch']-start_epoch)/total_epoch*0.7
            ...
            triplet_loss = nn.TripletMarginLoss(margin=0.1)( 
                            anchor_features[-1], mask_features, crp_mask_features)
        else:
        ...
        r0 = self.last_conv(r1)
        bf_mask = torch.sigmoid(r0) 
        ...
        # weight = torch.cat((x['image'], x['image'].max()-x['image']), dim=1)
        ...
        # tex_weight = x['image']
        # tex_weight = tex_weight / 2 + 0.5
        # topo_weight = 1 - tex_weight
        ...
        fg_sum = torch.sum(fg, dim=1, keepdims=True)
