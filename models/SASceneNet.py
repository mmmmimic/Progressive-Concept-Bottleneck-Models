...
from .unet_family import RegUNet, DTUNet
...
class SASceneNet(nn.Module):
    ...
    def __init__(self, arch, scene_classes, semantic_classes=151, in_channel=3):
        """
        ...
        if arch == 'ResNet-18':
            # ResNet-18 Network
            base = resnet.resnet18(weights=resnet.ResNet18_Weights.DEFAULT)
            ...
            # ResNet-50 Network
            base = resnet.resnet50(weights=resnet.ResNet50_Weights.DEFAULT)
...
class SASNet(SASceneNet):
    def __init__(self, arch, scene_classes, semantic_classes=151, in_channel=3, seg_model_path='logs/nnn1/model.t7'):
        """
        Initialization of the network
        :param arch: Desired backbone for RGB branch. Either ResNet-18 or ResNet-50
        :param scene_classes: Number of scene classes in the dataset.
        :param semantic_classes: Number of semantic classes. This should not be changed unless other semantic
                                 segmentation model is used
        """
        super().__init__(arch, scene_classes, semantic_classes, in_channel)
        self.semantic_classes = semantic_classes

        # revise the segmentation model here
        self.seg_model = DTUNet(in_channel, semantic_classes+1).cuda()
    ...
    def forward(self, inputs):
        ...
        with torch.no_grad():
            self.seg_model.eval()
            sem = self.seg_model(inputs)['logit']
        # sem = inputs['mask']
        # sem = torch.nn.functional.one_hot(sem, num_classes=self.semantic_classes+1).permute(0,3,1,2)
        # sem = sem.float()

        act, e7, act_rgb, act_sem = super().forward(x=x, sem=sem)
        return {'logit': act}
...
if __name__ == '__main__':
    image = torch.rand(2, 3, 224, 288)
    mask = torch.rand(2, 224, 288)
    mask = (mask > 0).long() 
    model = SASNet(arch='ResNet-18', scene_classes=8, semantic_classes=13)
    logit = model({'image':image, 'mask':mask})
